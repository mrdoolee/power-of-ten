...
const STEPS = [
  ...
  { exp: -9, title: '원자가 뭉친 순간', compare: '수소 원자 몇 개를 나란히 놓은 정도(약 1nm)', paragraphs: [
    '원자 몇 개가 서로 뭉쳐 있는 아주 작은 규모입니다. 이 정도 크기에서는 개별 원자들이 뭉쳐 만든 작은 분자나 원자단이 둥근 알갱이처럼 보이기 시작합니다.',
    '이렇게 작은 세계는 일반 카메라로 찍을 수 없습니다. 과학자들은 원자현미경(AFM)이나 전자현미경으로 원자 하나하나의 위치를 더듬어 지도처럼 그려내는 방식으로 이 세계를 관찰합니다.'] },
  { exp: -8, title: 'DNA 이중나선', compare: 'DNA 두 가닥 나선의 지름(약 2nm)', paragraphs: [
    '생명의 설계도인 DNA의 이중나선 구조가 뚜렷하게 드러나는 규모입니다. DNA는 인산, 당, 염기로 이루어진 뉴클레오타이드가 사슬처럼 이어져 두 가닥이 나선형으로 꼬인 구조를 하고 있습니다. 이 나선의 지름은 약 2nm로, 지금 화면 너비(10nm)의 5분의 1 정도를 차지합니다.',
  ...
  { exp: 1, title: '담요를 벗어나 공원으로', compare: '3층 건물 높이 정도', paragraphs: [
    '피크닉 자리를 둘러싼 잔디밭이 시야에 들어옵니다. 돗자리 위에 놓인 사람과 음식들은 아직 뚜렷하게 알아볼 수 있습니다.',
    '겨우 10배 커졌을 뿐인데 우리가 인식하는 "세계"는 벌써 달라지기 시작합니다. 방금 전까지 보이던 책의 글자나 과일의 질감은 더 이상 보이지 않고, 대신 돗자리 전체와 그 주변 잔디밭의 공간감이 눈에 들어옵니다.'] },
  { exp: 2, title: '공원 전체', compare: '축구장 하나 길이 정도', paragraphs: [
    '공원의 잔디밭 전체와 양옆의 도로, 자동차, 요트 선착장까지 한눈에 들어오는 규모입니다. 방금까지 뚜렷했던 돗자리 위 사람은 이제 풍경 속 작은 점 하나로 보일 뿐입니다.',
  ...
  { exp: 8, title: '점이 되어가는 지구', compare: '지구 지름(약 1만 2700km)의 약 8배 되는 시야', paragraphs: [
    '지구를 완전히 벗어나면서 표면의 세부 모습은 더 이상 보이지 않습니다. 대륙과 구름 무늬 대신, 별들 사이에 떠 있는 작고 푸른 점 하나만 남습니다.',
    '이때부터는 지구의 "모양"보다 "위치"가 더 중요한 정보가 됩니다. 실제로 태양계 탐사선 보이저 1호가 1990년, 태양계 끝자락에서 뒤돌아 찍은 지구 사진 "창백한 푸른 점(Pale Blue Dot)"도 바로 이런 모습이었습니다.'] },
  { exp: 9, title: '지구-달 시스템', compare: '지구-달 평균 거리(약 38만km)의 약 2.6배 되는 시야', paragraphs: [
    '지구와 달이 만드는 작은 시스템 전체가 시야에 들어옵니다. 가느다란 타원 궤도선은 달이 지구를 한 바퀴 도는 길을 나타내고, 그 중심의 점은 이제 하나로 겹쳐 보이는 지구와 달입니다.',
  ...
  { exp: 10, title: '태양에 가까운 행성들', compare: '수성 공전 궤도 반지름 정도', paragraphs: [
    '태양에 가장 가까운 행성인 수성의 공전 궤도가 시야에 들어오는 규모입니다. 태양의 강한 중력이 지배하는 영역입니다. 참고로 태양 자체의 지름은 약 139만km로, 이 화면 너비의 70분의 1 정도밖에 되지 않는 작은 점입니다.',
  ...
  { exp: 25, title: '초은하단 사이의 빈 공간', compare: '초은하단 사이 보이드(void)의 스케일', paragraphs: [
    '은하 몇 개가 희미하게 뭉쳐 보일 뿐, 화면 대부분은 텅 비어 있습니다. 초은하단과 초은하단 사이에 펼쳐진 거대한 빈 공간, "보이드(void)"의 스케일입니다.',
    '우주에는 물질이 거의 없는 이런 텅 빈 영역이 은하가 밀집된 영역보다 훨씬 넓게 펼쳐져 있습니다. 여기서 한 번 더 확대하면, 이렇게 성겨 보이던 구조가 사실은 거대한 그물망의 일부였다는 것이 비로소 드러납니다.'] },
  { exp: 26, title: '우주의 거미줄 구조', compare: '관측 가능한 우주의 지평선 규모', paragraphs: [
    '은하들이 실처럼 이어진 필라멘트와 그 사이의 거대한 빈 공간(보이드)이 대비되어 보입니다. 이 거대한 그물 무늬 하나하나가 은하 수천억 개가 모인 초은하단이며, 우주는 균일하지 않고 거미줄 같은 구조로 짜여 있습니다.',
    '이 거대한 그물 구조를 "우주 거대구조(cosmic web)"라 부릅니다. 초기 우주의 아주 미세한 밀도 차이가 중력에 의해 점점 증폭되며 물질이 특정 영역으로 모여든 결과입니다. 인류가 빛으로 관측할 수 있는 우주의 한계, Powers of Ten 여행은 여기, 우리가 알 수 있는 우주의 끝에서 마무리됩니다.'] },
];
